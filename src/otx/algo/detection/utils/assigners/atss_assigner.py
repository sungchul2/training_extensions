...
"""Implementation modified from mmdet.models.task_modules.assigners.atss_assigner.

Reference : https://github.com/open-mmlab/mmdetection/blob/v3.2.0/mmdet/models/task_modules/assigners/atss_assigner.py
"""
...
class ATSSAssigner(BaseAssigner):
    ...
    def assign(
        ...
        Args:
            pred_instances (InstaceData): Instances of model
                predictions. It includes ``priors``, and the priors can
                be anchors, points, or bboxes predicted by the model,
                shape(n, 4).
            num_level_priors (List): Number of bboxes in each level
            gt_instances (InstaceData): Ground truth of instance
                annotations. It usually includes ``bboxes`` and ``labels``
                attributes.
            gt_instances_ignore (InstaceData, optional): Instances
        ...
        candidate_idxs_list = []
        start_idx = 0
        for priors_per_level in num_level_priors:
            ...
            candidate_idxs_list.append(topk_idxs_per_level + start_idx)
            start_idx = end_idx
        candidate_idxs = torch.cat(candidate_idxs_list, dim=0)
        ...
        priors_cx = (priors[:, 0] + priors[:, 2]) / 2.0
        priors_cy = (priors[:, 1] + priors[:, 3]) / 2.0
        ep_priors_cx = priors_cx.view(1, -1).expand(num_gt, num_priors).contiguous().view(-1)
        ep_priors_cy = priors_cy.view(1, -1).expand(num_gt, num_priors).contiguous().view(-1)
